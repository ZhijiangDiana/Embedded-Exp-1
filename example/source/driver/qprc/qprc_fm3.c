...
static void QPRCIrqHandler(uint8_t Ch, uint16_t IrqValue);
...
{
    /* Check parameter */
    ASSERT((QPRC_CH0 == Ch) || (QPRC_CH1 == Ch));
 ...
 ******************************************************************************
 */
void QPRC_IRQHandler(void)
{
    uint16_t irq;

    /* deviation from MISRA-C:2004 Rule 11.3 */
    irq = (uint16_t)(FM3_INTREQ->IRQ06MON);

    if((irq & 0x00FC) != 0)
    {
        QPRCIrqHandler(QPRC_CH0,irq);
    }

    if((irq & 0x3F00) != 0)
    {
        QPRCIrqHandler(QPRC_CH1,irq);
    }

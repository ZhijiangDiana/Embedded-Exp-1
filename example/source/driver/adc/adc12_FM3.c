 ...
 ** \arg   ADC_UNIT1
 ** \arg   ADC_UNIT2 
 ** \param CT Compare time
 **
 ** \return None
 **
 ******************************************************************************
 */
void ADC12_SetCompareTime(uint8_t Unit,  ADC12_CmpTimeDivT CT)
{
    /* Check the parameter */
    ASSERT(IS_ADC_UNIT_VALID(Unit));
    ASSERT(IS_ADC_CMP_TIME_VALID(CT));
    
    ADC12Reg[Unit] ->ADCT &= ~(7ul);
    ADC12Reg[Unit] ->ADCT |= CT;
    return;
}
...
void ADC0_IRQHandler(void)
{
    ...
    ADC12_IrqHandler(ADC12_UNIT0, irq);
}
...
void ADC1_IRQHandler(void)
{
    ...
    ADC12_IrqHandler(ADC12_UNIT1, irq);
}
...
void ADC2_IRQHandler(void)
{
    ...
    ADC12_IrqHandler(ADC12_UNIT2, irq);
}
